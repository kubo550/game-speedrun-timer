const returnTime = (time: number) => (time > 10 ? time : `0${time}`);
const createAndAddSpan = (classname: string) => {
...
const displayTime = (minute: number, second: number, millisecond: number) => {
  const timerEl = document.querySelector(".timer");
  if (!timerEl) {
    return alert('Something with timer went wrong!')
  }

  timerEl.textContent = `${minute}:${returnTime(second)}.${returnTime(
    millisecond
  )}`;
};
...
const checkLimit = (limit: number, className: string) => {
